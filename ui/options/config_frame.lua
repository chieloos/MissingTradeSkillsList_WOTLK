...
MTSLOPTUI_CONFIG_FRAME = {
    ...
    ChangeValue = function (self, value_name, value, text)
        ...
        MTSLUI_LIBS.DD:UIDropDownMenu_SetText(self.ui_frame[value_name .. "_drop_down"], text)
    end,

    ChangeWithSubValue = function (self, value_name, sub_value_name, value, text)
        self.config_values[value_name][sub_value_name] = value
        MTSLUI_LIBS.DD:UIDropDownMenu_SetText(self.ui_frame[value_name .. "_" .. sub_value_name .. "_drop_down"], text)
    end,
    ...
    ResetUI = function (self)
        ...
        MTSLUI_LIBS.DD:UIDropDownMenu_SetText(self.ui_frame.minimap_shape_drop_down, MTSLUI_TOOLS:GetLocalisedLabel(self.config_values.minimap.shape))
        MTSLUI_LIBS.DD:UIDropDownMenu_SetText(self.ui_frame.minimap_radius_drop_down, self.config_values.minimap.radius .. " px")

        -- Enchanced Tooltip
        self:ShowValueInCheckBox(self.tooltip_check, self.config_values.tooltip.active)
        MTSLUI_LIBS.DD:UIDropDownMenu_SetText(self.ui_frame.tooltip_faction_drop_down, MTSLUI_TOOLS:GetLocalisedLabel(self.config_values.tooltip.faction))
        MTSLUI_LIBS.DD:UIDropDownMenu_SetText(self.ui_frame.tooltip_known_drop_down, MTSLUI_TOOLS:GetLocalisedLabel(self.config_values.tooltip.known))
        -- Link to chat
        self:ShowValueInCheckBox(self.linktochat_check, self.config_values.chat.active)
        MTSLUI_LIBS.DD:UIDropDownMenu_SetText(self.ui_frame.chat_channel_drop_down, MTSLUI_TOOLS:GetLocalisedLabel(self.config_values.chat.channel))

        MTSLUI_LIBS.DD:UIDropDownMenu_SetText(self.ui_frame.location_mtsl_button_drop_down, MTSLUI_TOOLS:GetLocalisedLabel(string.lower(self.config_values.location_mtsl.button)))
        MTSLUI_LIBS.DD:UIDropDownMenu_SetText(self.ui_frame.location_mtsl_frame_drop_down, MTSLUI_TOOLS:GetLocalisedLabel(string.lower(self.config_values.location_mtsl.frame)))

        -- Split modes
        for _, k in pairs(self.ui_split_keys) do
            MTSLUI_LIBS.DD:UIDropDownMenu_SetText(self.ui_frame["ui_split_" .. k .. "_drop_down"], self.config_values.ui_split[k])
        end
        -- UI scales
        for _, k in pairs(self.ui_scale_keys) do
            MTSLUI_LIBS.DD:UIDropDownMenu_SetText(self.ui_frame["ui_scale_" .. k .. "_drop_down"], (100 * self.config_values.ui_scale[k]) .. " %")
        end
        -- Font
        MTSLUI_LIBS.DD:UIDropDownMenu_SetText(self.ui_frame.font_name_drop_down, MTSL_TOOLS:GetItemFromArrayByKeyValue(self.drop_down_lists.font.names, "id", self.config_values.font.name).name)
        MTSLUI_LIBS.DD:UIDropDownMenu_SetText(self.ui_frame.font_title_drop_down, self.config_values.font.title)
        MTSLUI_LIBS.DD:UIDropDownMenu_SetText(self.ui_frame.font_label_drop_down, self.config_values.font.label)
        MTSLUI_LIBS.DD:UIDropDownMenu_SetText(self.ui_frame.font_text_drop_down, self.config_values.font.text)
    end,
